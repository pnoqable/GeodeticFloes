...
def calcRejection( vertI, vertJ, i, j ):
    d = vertI - vertJ
    lendsquare = np.inner( d, d )
    lend = np.sqrt( lendsquare )
    normd = d / lend if lend else d
    rejection = normd / max( 0.1, lendsquare )
    return ( i, j, rejection, lend )

def project( pos, vel ):
    n = norm( pos )
    return vel - np.inner( n, vel ) * n

def calcVertex( i, rejectionsI, vertI, transI ):
    transI += 0.01 * np.sum( rejectionsI, axis=0 )
    transI = 0.9 * project( vertI, transI )
    vertI = norm( vertI + transI )
    return( i, transI, vertI )
    ...
    while state.running:
        ...
        for i in range( vertices.shape[0] ):
            for j in range( i+1, vertices.shape[0] ):
                futures.append( executor.submit( calcRejection, vertices[i], vertices[j], i, j ) )

        for f in futures:
            ( i, j, rejection, lend ) = f.result()
            rejections[i,j] =  rejection
            rejections[j,i] = -rejection
            state.dmin = min( state.dmin, lend )
            state.dmax = max( state.dmax, lend )

        futures = list()
        for i in range( vertices.shape[0] ):
            futures.append( executor.submit( calcVertex, i, rejections[i,:], vertices[i], translations[i] ) )

        for f in futures:
            ( i, transI, vertI ) = f.result()
            translations[i] = transI
            vertices[i] = vertI
